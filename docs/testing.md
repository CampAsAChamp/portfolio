...
The pipeline runs on every push and pull request via [`.github/workflows/test-and-deploy.yml`](../.github/workflows/test-and-deploy.yml):

**Test job:** lint → unit tests → build → E2E → upload `build/` artifact → (on `main` push) deploy

**Lighthouse job** ([`lighthouse.yml`](../.github/workflows/lighthouse.yml), after Test and Deploy passes on `main`): desktop + mobile audits, artifact upload, GitHub status checks.
