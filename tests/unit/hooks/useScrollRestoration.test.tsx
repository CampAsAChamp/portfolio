...
import { SCROLL_RESTORING_ATTR, SCROLL_STORAGE_KEY } from "utils/scrollRestoration"
...
describe("useScrollRestoration", () => {
  ...
  it("restores a saved scroll position on mount", () => {
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "1200")
  ...
  it("stops restoring when scroll diverges from the saved position", () => {
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("500")
  ...
  it("keeps restoring when scroll is clamped below the saved position during lazy load", () => {
    const rafCallbacks: FrameRequestCallback[] = []
    vi.stubGlobal("requestAnimationFrame", (cb: FrameRequestCallback) => {
      rafCallbacks.push(cb)
      return rafCallbacks.length
    ...
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "3500")
    renderHook(() => useScrollRestoration())
    scrollTo.mockClear()

    Object.defineProperty(window, "scrollY", { configurable: true, writable: true, value: 700 })
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("3500")
    expect(disconnect).not.toHaveBeenCalled()

    Object.defineProperty(document.documentElement, "scrollHeight", { configurable: true, value: 4500 })
    act(() => {
      mutationCallback?.([], {} as MutationObserver)
    })

    expect(scrollTo).toHaveBeenCalledWith(0, 3500)
  })
  ...
  it("does not overwrite the saved position while restoring", () => {
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("1200")
  })

  it("re-applies the saved position when the DOM mutates during restore", () => {
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "800")
  ...
  it("reveals in-viewport animated elements after restore", () => {
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "500")
  ...
  it("clears the restore-pending attribute when restore finishes", () => {
    vi.useFakeTimers()
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "500")
  ...
  it("prefers the saved scroll position over the URL hash", () => {
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "450")
  ...
  it("persists scroll position while the user scrolls after restore", () => {
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("640")
  })
  ...
  it("persists scroll position on pagehide", () => {
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("960")
  })

  it("preserves the intended saved Y on pagehide during restore", () => {
    sessionStorage.setItem(SCROLL_STORAGE_KEY, "2200")
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("2200")
  })
  ...
  it("persists the locked modal scroll offset instead of window.scrollY", () => {
    ...
    expect(sessionStorage.getItem(SCROLL_STORAGE_KEY)).toBe("1500")
  })
