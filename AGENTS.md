...
yarn exp:ui                # Local experience content editor (YAML sync)
yarn exp:sync              # Generate portfolio + LinkedIn/resume exports
```
...
experience-sync/  # Experience sync source of truth + local editor (see experience-sync/README.md)
tests/
  unit/           # Vitest + Testing Library (mirrors src/)
  e2e/            # Playwright (desktop/, mobile/, fixtures/, helpers/)
config/           # Tooling configs (eslint, prettier, vite, lighthouse, etc.)
```

## Conventions (summary)

- Named exports: `export function ComponentName(): React.ReactElement`
- Absolute imports from `src` (no `../` across top-level dirs) — aliases in `tsconfig.json`
...
| Tooling configs | `config/README.md` |
...
| Experience sync | `experience-sync/README.md` (`yarn exp:ui`) |
| CI/CD workflows | `.cursor/rules/ci-workflows.mdc` |
| Content/data files | `.cursor/rules/data-and-content.mdc` |
| Mobile Lighthouse performance | `docs/lighthouse-mobile-perf.md` |
| Full conventions | `.cursor/rules/` |
